...
let touchCommand =
  process.platform == "win32"
    ? `.>/assets/db/settings.db`
    : `touch /assets/db/settings.db`;
        ...
        if (error) {
          ...
          exec(
            `git clone https://github.com/${
              usernames[index]
            }/${extractedAssignment} ${workDir}/${studentFolder}/${extractedAssignment}`,
            err => {
              if (err) console.log(err);
            }
          );
...
const handleClone = event => {
  ...
  const studentFolder = eventParent
    .querySelector(".studentInput")
    .value.split(" ")
    .join("_");
  const studentUsername = eventParent.querySelector(".studentUsername").value;

  exec(
    `cd ${workDir}\\${studentFolder}\\${extractedAssignment} && git pull`,
    err => {
      if (err) console.log(err);
    }
  );
...
const handleSandbox = event => {
  ...
  shell.openExternal(
    `https://codesandbox.io/s/github/${currentUsername}/${extractedAssignment}`
  );
};
